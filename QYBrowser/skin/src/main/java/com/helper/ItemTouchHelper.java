...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    ConcurrentHashMap<Integer, AnimatorSet> hashMap = new ConcurrentHashMap<>();
    ...
    private boolean mSelectAnimEnd;
    /**
     ...
     */
    private void select(ViewHolder selected, final int actionState) {
                ...
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType,
                    ...
                    public void onAnimationStart(ValueAnimatorCompat animation) {
                        ...
                        if (manager != null && manager instanceof GridLayoutManager) {
                            ViewCompat.setPivotX(prevSelected.itemView, prevSelected.itemView.getWidth() / 2);
                            ViewCompat.setPivotY(prevSelected.itemView, prevSelected.itemView.getHeight() / 2);
                            ViewCompat.postInvalidateOnAnimation(prevSelected.itemView);
                            AnimatorSet animatorSet = hashMap.get(prevSelected.itemView.hashCode());
                            if (animatorSet == null) {
                                animatorSet = new AnimatorSet();
                                animatorSet.playTogether(
                                        ObjectAnimator.ofFloat(prevSelected.itemView, "scaleX", 1.2f, 1.0f),
                                        ObjectAnimator.ofFloat(prevSelected.itemView, "scaleY", 1.2f, 1.0f)
                                );
                                animatorSet.setDuration(250);
                                animatorSet.setInterpolator(new FastOutLinearInInterpolator());
                                hashMap.put(prevSelected.itemView.hashCode(), animatorSet);
                            }

                            if (prevSelected.itemView.getScaleX() >= 1.0 && !animatorSet.isRunning()) {
                                animatorSet.start();
        ...
        public ViewHolder chooseDropTarget(ViewHolder selected,
            ...
            for (int i = 0; i < targetsSize; i++) {
                ...
                if (dx > 0) {
                    ...
                    if (diff < target.itemView.getWidth() * 0.4 && target.itemView.getRight() > selected.itemView.getRight()) {
                ...
                if (dx < 0) {
                    ...
                    if (diff > -target.itemView.getWidth() * 0.4 && target.itemView.getLeft() < selected.itemView.getLeft()) {
                        final int score = Math.abs(diff);
                        if (score > winnerScore) {
                            winnerScore = score;
                            winner = target;
                ...
                if (dy < 0) {
                    ...
                    if (diff > -target.itemView.getHeight() * 0.4 && target.itemView.getTop() < selected.itemView.getTop()) {
                        final int score = Math.abs(diff);
                        if (score > winnerScore) {
                            winnerScore = score;
                            winner = target;
                ...
                if (dy > 0) {
                    ...
                    if (diff < target.itemView.getHeight() * 0.4 && target.itemView.getBottom() > selected.itemView.getBottom()) {
                        final int score = Math.abs(diff);
                        if (score > winnerScore) {
                            winnerScore = score;
                            winner = target;
